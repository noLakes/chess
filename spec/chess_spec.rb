...
describe Game do
  ...
  describe "#sim_game" do
    ...
    it "returns a clone of the given board" do
      sim = test.sim_game
      expect(board.object_id).not_to eql(sim.board.object_id)
    end

    it "returns a deep clone of the board" do
      sim = test.sim_game
      expect(board['a', 1].object_id).not_to eql(sim.board['a', 1].object_id)
    end
  ...
  describe "#check_mate" do
    ...
    it "returns true for queen/knight scenario" do
      test.turn = test.player[2]
      board['e', 8].piece = King.new('B', ['e', 8])
      board['a', 1].piece = King.new('W', ['a', 1])
